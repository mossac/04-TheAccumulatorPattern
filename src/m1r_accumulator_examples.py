...
#
#              for k in range(r - s):- r to s so should be s-r not r-s
#                  count = 0 -needs to be outside loop
#                  if math.cos(r) > 0: -no ()
#                      count = 1 -count=count +1 not count = 1
#                  return count -terminates function so should be outside loop
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean? loops but says you arent using variable
#
